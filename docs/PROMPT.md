...
6. docs/env.md — run environment + programmatic login; config/.env holds LFM_EMAIL / LFM_PASSWORD
...
- (An unattended headless path exists — bin/run-case.sh + run-batches.sh + harvest.sh — for the
  future CI phase (Jenkins on-prem). It follows these same rules with a hard per-case watchdog.)
...
1. INGEST: fetch the ticket via the Atlassian MCP (Xray steps live in custom fields
   ...
   cases/QA-<id>.md in the standard frontmatter + Steps + Assertions format.
...
═══ OUTPUT ═══
- One report per case: results/<YYYY-MM-DD>/QA-<id>-report.md with steps executed, an assertions table
