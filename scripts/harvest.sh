...
#
# Reads runs/<date>/summary.json + the case reports and applies PROMPT.md's post-pass
...
#
# Usage:  scripts/harvest.sh                 # today's run
#         scripts/harvest.sh 2026-06-27      # a specific run date
set -uo pipefail
cd "$(dirname "$0")/.."
...
if [[ ! -f "$SUMMARY" ]]; then
  echo "ERROR: no summary at ${SUMMARY} — run scripts/run-batches.sh first." >&2
  exit 1
fi
...
Read first, in order:
1. PROMPT.md — apply the rules in its OUTPUT section about skills/registry/KB maintenance.
